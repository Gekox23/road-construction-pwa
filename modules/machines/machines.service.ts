...
export async function listMachines(search?: string): Promise<Machine[]> {
  try {
    if (search) {
...
export async function createMachine(data: Partial<Machine>, userId: string) {
  try {
    const res = await db.query(
...
export async function updateMachine(id: string, data: Partial<Machine>, userId: string) {
  try {
...
export async function addHourLog(machineId: string, eventDate: string, hourValue: number, photoUrl: string | null, notes: string | null, userId: string) {
  try {
    const res = await db.query(
...
export async function addFuelLog(machineId: string, eventDate: string, liters: number, location: string | null, userId: string) {
  try {
    const res = await db.query(
...
export async function getMachineHistory(machineId: string) {
  try {
    const [hours, fuel, workorders] = await Promise.all([
      db.query('SELECT * FROM machine_hour_logs WHERE machine_id=$1 ORDER BY event_date DESC', [machineId]),
      db.query('SELECT * FROM machine_fuel_logs WHERE machine_id=$1 ORDER BY event_date DESC', [machineId]),
      db.query('SELECT * FROM workorders WHERE machine_id=$1 ORDER BY event_date DESC', [machineId]),
    ]);
