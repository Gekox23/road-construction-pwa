...
export async function createOrder(data: { items: { itemName: string; quantity: number; unit?: string; unknownItem?: boolean }[]; notes?: string; eventDate: string }, userId: string) {
  try {
...
export async function approveOrder(id: string, approved: boolean, userId: string) {
  try {
    const status = approved ? 'jovahagyva' : 'elutasitva';
    const res = await db.query('UPDATE orders SET status=$1, updated_at=NOW() WHERE id=$2 RETURNING *', [status, id]);
